...
class reinforcement_net(nn.Module):
    ...
    def __init__(self, use_cuda):  # , snapshot=None
        ...
        self.num_rotations = 16  # change from 16
    ...
    def forward(self, input_color_data, input_depth_data, is_volatile=False, specific_rotation=-1, primitive=None, rotate_theta=0):
        ...
        if is_volatile:
            ...
            # Compute intermediate features
            if primitive == "push":
                interm_color_feat = self.push_color_trunk.features(rotate_color)
                interm_depth_feat = self.push_depth_trunk.features(rotate_depth)
                interm_feat = torch.cat((interm_color_feat, interm_depth_feat), dim=1)
            elif primitive == "grasp":
                interm_color_feat = self.grasp_color_trunk.features(rotate_color)
                interm_depth_feat = self.grasp_depth_trunk.features(rotate_depth)
                interm_feat = torch.cat((interm_color_feat, interm_depth_feat), dim=1)
            ...
            if self.use_cuda:
                flow_grid_after = F.affine_grid(Variable(affine_mat_after, requires_grad=False).cuda(), interm_feat.data.size())
            else:
                flow_grid_after = F.affine_grid(Variable(affine_mat_after, requires_grad=False), interm_feat.data.size())

                # Forward pass through branches, undo rotation on output predictions, upsample results
                # output_prob.append([nn.Upsample(scale_factor=4, mode='bilinear').forward(F.grid_sample(self.pushnet(interm_push_feat), flow_grid_after, mode='nearest')),
                #                     nn.Upsample(scale_factor=4, mode='bilinear').forward(F.grid_sample(self.graspnet(interm_grasp_feat), flow_grid_after, mode='nearest'))])
            # print('interm_feat type', type(interm_feat))
            # print('interm_feat type', interm_feat.dtype)
            # print('interm_feat shape', interm_feat.shape)
            # output_prob = nn.Upsample(scale_factor=16, mode='bilinear').forward(F.grid_sample(self.pushnet(interm_feat), flow_grid_after, mode='nearest'))
            if primitive == "push":
                output_prob = nn.Upsample(scale_factor=16, mode='bilinear').forward(F.grid_sample(self.pushnet(interm_feat), flow_grid_after, mode='nearest'))
            elif primitive == "grasp":
                output_prob = nn.Upsample(scale_factor=16, mode='bilinear').forward(F.grid_sample(self.graspnet(interm_feat), flow_grid_after, mode='nearest'))
