...
from models import reactive_net, reinforcement_net, reinforcement2d_net, reinforcement6d_net
...
class Trainer(object):
    def __init__(self, method, push_rewards, future_reward_discount,
        ...
        # Fully convolutional Q network for deep reinforcement learning
        ...
        elif self.method == 'reinforcement2d':
            self.model = reinforcement2d_net(self.use_cuda)
            # self.model = reinforcement_net(self.use_cuda)
            self.push_rewards = push_rewards
            self.future_reward_discount = future_reward_discount
        ...
        elif self.method == 'reinforcement6d':
            self.model = reinforcement6d_net(self.use_cuda)
            self.push_rewards = push_rewards
            self.future_reward_discount = future_reward_discount
            ...
            grasp_class_weights[grasp_num_classes - 1] = 0
            if self.use_cuda:
                self.grasp_fc_criterion = CrossEntropyLoss2d().cuda()
                self.grasp_heat_criterion = torch.nn.SmoothL1Loss(reduce=False).cuda()
                self.push_criterion = torch.nn.SmoothL1Loss(reduce=False).cuda()
            else:
                self.grasp_fc_criterion = CrossEntropyLoss2d()
                self.grasp_heat_criterion = torch.nn.SmoothL1Loss(reduce=False)
                self.push_criterion = torch.nn.SmoothL1Loss(reduce=False)  # Huber loss
    ...
    # forward function for reinforce2d_net
    # def forward(self, push_samples, grasp_samples, is_volatile=False, specific_rotation=-1):
    #     # Pass input data through model
    #     push_predictions, grasp_predictions, state_feat = self.model.forward(push_samples, grasp_samples, is_volatile, specific_rotation)
    #
    #     return push_predictions, grasp_predictions, state_feat

    # forward function for reinforce6d_net
    def forward(self, color_heightmap, depth_heightmap, grasp_samples, is_volatile=False, specific_rotation=-1):
        # Pass input data through model
        ...
        # print ("\ntrainer.py forward() after image preprocessing")

        push_output_prob, grasp_predictions, grasp_output_heat, state_feat = self.model.forward(input_color_data, input_depth_data, grasp_samples, is_volatile, specific_rotation)

        if self.method == 'reinforcement6d':
            # Return Q values (and remove extra padding)
            for rotate_idx in range(len(push_output_prob)):
                if rotate_idx == 0:
                    push_predictions = push_output_prob[rotate_idx][0].cpu().data.numpy()[:, 0,
                                       int(padding_width / 2):int(color_heightmap_2x.shape[0] / 2 - padding_width / 2),
                                       int(padding_width / 2):int(color_heightmap_2x.shape[0] / 2 - padding_width / 2)]
                    # print(grasp_output_heat[rotate_idx][0])
                else:
                    push_predictions = np.concatenate((push_predictions,
                                                       push_output_prob[rotate_idx][0].cpu().data.numpy()[:, 0,
                                                       int(padding_width / 2):int(
                                                           color_heightmap_2x.shape[0] / 2 - padding_width / 2),
                                                       int(padding_width / 2):int(
                                                           color_heightmap_2x.shape[0] / 2 - padding_width / 2)]),
                                                      axis=0)
            grasp_heat_pred = grasp_output_heat[0][0].cpu().data.numpy()[:, 0,
                              int(padding_width / 2):int(color_heightmap_2x.shape[0] / 2 - padding_width / 2),
                              int(padding_width / 2):int(color_heightmap_2x.shape[0] / 2 - padding_width / 2)]
            # print(grasp_heat_pred.shape)
            for grasp_ind in range(len(grasp_samples)):
                grasp = grasp_samples[grasp_ind]
                ind_x = min(int((grasp[0] + 0.724) / 0.002), 223)
                ind_y = min(int((grasp[1] + 0.224) / 0.002), 223)
                grasp_predictions[grasp_ind] = grasp_predictions[grasp_ind] + grasp_heat_pred[0][ind_x][ind_y]

        return push_predictions, grasp_predictions, state_feat

    # forward function for reinforce_net
    ...
    def get_label_value(self, primitive_action, push_success, grasp_success, change_detected, prev_push_predictions,
                        prev_grasp_predictions, next_color_heightmap, next_depth_heightmap, next_grasp_samples):
        ...
        if self.method == 'reactive':
            ...
            if not change_detected and not grasp_success:
                ...
                next_push_predictions, next_grasp_predictions, next_state_feat = self.forward(next_color_heightmap,
    ...
    # def backprop(self, push_sample, grasp_sample, primitive_action, prev_best_pose_ind, label_value):
    def backprop(self, color_heightmap, depth_heightmap, grasp_samples, primitive_action, best_pix_ind, label_value):

        loss_value = 0.0
        # print("backprop: ", grasp_samples, best_pix_ind, label_value)
        if self.method == 'reactive':
            ...
            if primitive_action == 'push':
                ...
                # Do forward pass with specified rotation (to save gradients)
                push_predictions, grasp_predictions, state_feat = self.forward(color_heightmap, depth_heightmap, grasp_samples, is_volatile=False, specific_rotation=best_pix_ind[0])

                if self.use_cuda:
                    loss = self.push_criterion(self.model.push_output_prob[0][0].view(1,320,320), Variable(torch.from_numpy(label).float().cuda())) * Variable(torch.from_numpy(label_weights).float().cuda(),requires_grad=False)
                else:
                    loss = self.push_criterion(self.model.push_output_prob[0][0].view(1,320,320), Variable(torch.from_numpy(label).float())) * Variable(torch.from_numpy(label_weights).float(),requires_grad=False)
                ...
                fc_label = np.zeros(grasp_samples.shape[0])
                fc_label[best_pix_ind] = 0.99

                grasp = grasp_samples[best_pix_ind]
                ind_x = min(int((grasp[0] + 0.724) / 0.002), 223)
                ind_y = min(int((grasp[1] + 0.224) / 0.002), 223)

                # Compute labels
                heat_label = np.zeros((1, 320, 320))
                action_area = np.zeros((224, 224))
                action_area[ind_x][ind_y] = 0.99
                tmp_label = np.zeros((224, 224))
                tmp_label[action_area > 0] = 0.99
                heat_label[0, 48:(320 - 48), 48:(320 - 48)] = tmp_label

                # Compute label mask
                label_weights = np.zeros(heat_label.shape)
                tmp_label_weights = np.zeros((224, 224))
                tmp_label_weights[action_area > 0] = 1
                label_weights[0, 48:(320 - 48), 48:(320 - 48)] = tmp_label_weights

                push_predictions, grasp_predictions, state_feat = self.forward(color_heightmap, depth_heightmap, grasp_samples, is_volatile=False, specific_rotation=0)

                if self.use_cuda:
                    output = Variable(torch.from_numpy(np.array(self.model.grasp_output_prob)).float().cuda(), requires_grad=True)
                    target = Variable(torch.from_numpy(fc_label).long().cuda(), requires_grad=False)
                    fc_loss = self.grasp_fc_criterion(output, target)
                    heat_loss = self.grasp_heat_criterion(self.model.grasp_output_heat[0][0].view(1, 320, 320), Variable(torch.from_numpy(heat_label).float().cuda())) * Variable(torch.from_numpy(label_weights).float().cuda(), requires_grad=False)
                else:
                    output = Variable(torch.from_numpy(np.array(self.model.grasp_output_prob)).float().cuda(), requires_grad=True)
                    target = Variable(torch.from_numpy(fc_label).long().cuda(), requires_grad=False)
                    fc_loss = self.grasp_fc_criterion(output, target)
                    heat_loss = self.grasp_heat_criterion(self.model.grasp_output_heat[0][0].view(1, 320, 320), Variable(torch.from_numpy(heat_label).float())) * Variable(torch.from_numpy(label_weights).float(), requires_grad=False)

                fc_loss.backward()
                heat_loss = heat_loss.sum()
                heat_loss.backward()
                loss_value = heat_loss.cpu().data.numpy()[0]
    ...
    def grasp_heuristic(self, depth_heightmap):
        ...
        for rotate_idx in range(num_rotations):
            ...
            valid_areas[np.logical_and(
                ...
                rotated_heightmap - ndimage.interpolation.shift(rotated_heightmap, [0, 25], order=0) > 0.02)] = 1
            # valid_areas = np.multiply(valid_areas, rotated_heightmap)
            blur_kernel = np.ones((25, 25), np.float32) / 9
            valid_areas = cv2.filter2D(valid_areas, -1, blur_kernel)
