...
class Trainer(object):
    def __init__(self, method, push_rewards, future_reward_discount,
        ...
        if load_snapshot:
            self.model.load_state_dict(torch.load(snapshot_file), strict=False)
    ...
    def forward(self, color_heightmap, depth_heightmap, is_volatile=False, specific_rotation=-1):
        ...
        num_rotations = 16
        push_predictions, grasp_predictions = None, None
        state_feat = []
        for rotate_idx in range(num_rotations):
            # print("rotate_idx:", rotate_idx)
            rotate_theta = np.radians(rotate_idx * (360 / num_rotations))
            # print("rotate_idx, rotate_theta: ",rotate_idx, rotate_theta)
            # print("rotate_idx:{}, rotate_theta:{}",rotate_idx, rotate_theta)
            push_prob, push_feat = self.model.forward(input_color_data, input_depth_data, is_volatile, specific_rotation,"push", rotate_theta)
            grasp_prob, grasp_feat = self.model.forward(input_color_data, input_depth_data, is_volatile, specific_rotation,"grasp", rotate_theta)
            # print("push_prob type: ", type(push_prob))
            # if isinstance(push_prob, list):
            #     print(len(push_prob[0]))
            #     print(push_prob[0][0].shape)
            #
            # print("push_prob.shape: ", push_prob.shape)
            # print("push_prob.dtype: ", push_prob.dtype)
            # push_prob = np.reshape(push_prob, [1, 1, IM_SIZE, IM_SIZE])
            # grasp_predictions = np.reshape(grasp_predictions, [1, num_rotations, 32, 32])
            if rotate_idx == 0:
                push_predictions = push_prob.cpu().data.numpy()[:, 0,
                                   int(padding_width / 2):int(color_heightmap_2x.shape[0] / 2 - padding_width / 2),
                                   int(padding_width / 2):int(color_heightmap_2x.shape[0] / 2 - padding_width / 2)]
                grasp_predictions = grasp_prob.cpu().data.numpy()[:, 0,
                                    int(padding_width / 2):int(color_heightmap_2x.shape[0] / 2 - padding_width / 2),
                                    int(padding_width / 2):int(color_heightmap_2x.shape[0] / 2 - padding_width / 2)]
            else:
                push_predictions = np.concatenate((push_predictions,
                                                   push_prob.cpu().data.numpy()[:, 0,
                                                   int(padding_width / 2):int(
                                                       color_heightmap_2x.shape[0] / 2 - padding_width / 2),
                                                   int(padding_width / 2):int(
                                                       color_heightmap_2x.shape[0] / 2 - padding_width / 2)]), axis=0)
                grasp_predictions = np.concatenate((grasp_predictions,
                                                    grasp_prob.cpu().data.numpy()[:, 0,
                                                    int(padding_width / 2):int(
                                                        color_heightmap_2x.shape[0] / 2 - padding_width / 2),
                                                    int(padding_width / 2):int(
                                                        color_heightmap_2x.shape[0] / 2 - padding_width / 2)]), axis=0)
            # print("push_predictions.shape:", push_predictions.shape)
            # print("grasp_predictions.shape:", grasp_predictions.shape)
            # !!! main.py not using state_feat
            # state_feat.append([push_feat, grasp_feat])

        # push_output_prob, push_state_feat = self.model.forward(input_color_data, input_depth_data, is_volatile, specific_rotation, "push")
        # grasp__output_prob, grasp_state_feat = self.model.forward(input_color_data, input_depth_data, is_volatile, specific_rotation, "grasp")
        ...
        #         if rotate_idx == 0:
        #             push_predictions = F.softmax(push_output_prob[rotate_idx][0], dim=1).cpu().data.numpy()[:,0,(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2),(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2)]
        ...
        #         else:
        #             push_predictions = np.concatenate((push_predictions, F.softmax(push_output_prob[rotate_idx][0], dim=1).cpu().data.numpy()[:,0,(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2),(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2)]), axis=0)
        ...
        #     for rotate_idx in range(len(push_output_prob)):
        #         if rotate_idx == 0:
        #             push_predictions = push_output_prob[rotate_idx][0].cpu().data.numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2)]
        ...
        #         else:
        #             push_predictions = np.concatenate((push_predictions, push_output_prob[rotate_idx][0].cpu().data.numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2)]), axis=0)
        ...
        # grasp_predictions = push_predictions
        # state_feat = [push_state_feat, grasp_state_feat]
    ...
    def backprop(self, color_heightmap, depth_heightmap, primitive_action, best_pix_ind, label_value):
        ...
        if self.method == 'reactive':
            ...
            # Compute labels
            label = np.zeros((1, IM_SIZE, IM_SIZE)) + fill_value
            ...
            label[0, 48:(IM_SIZE-48), 48:(IM_SIZE-48)] = tmp_label
            ...
            # Compute labels
            label = np.zeros((1, IM_SIZE, IM_SIZE))
            action_area = np.zeros((224, 224))
            action_area[best_pix_ind[1]][best_pix_ind[2]] = 1
            # blur_kernel = np.ones((5,5),np.float32)/25
            # action_area = cv2.filter2D(action_area, -1, blur_kernel)
            ...
            label_weights[0, 48:(IM_SIZE-48), 48:(IM_SIZE-48)] = tmp_label_weights
            ...
            if primitive_action == 'push':
                ...
                if self.use_cuda:
                    loss = self.criterion(self.model.output_prob[0][0].view(1, IM_SIZE, IM_SIZE), Variable(torch.from_numpy(label).float().cuda())) * Variable(torch.from_numpy(label_weights).float().cuda(),requires_grad=False)
                else:
                    loss = self.criterion(self.model.output_prob[0][0].view(1, IM_SIZE, IM_SIZE), Variable(torch.from_numpy(label).float())) * Variable(torch.from_numpy(label_weights).float(),requires_grad=False)
                loss = loss.sum()
                loss.backward()
                loss_value = loss.cpu().data.numpy()[0]
                ...
                if self.use_cuda:
                    loss = self.criterion(self.model.output_prob[0][1].view(1, IM_SIZE, IM_SIZE), Variable(torch.from_numpy(label).float().cuda())) * Variable(torch.from_numpy(label_weights).float().cuda(),requires_grad=False)
                else:
                    loss = self.criterion(self.model.output_prob[0][1].view(1, IM_SIZE, IM_SIZE), Variable(torch.from_numpy(label).float())) * Variable(torch.from_numpy(label_weights).float(),requires_grad=False)
                loss = loss.sum()
                loss.backward()
                loss_value = loss.cpu().data.numpy()[0]
