    ...
    while(True):
        ...
        while nl.norm(V) >=.01 and nl.norm(thetadot) >= .0001:
            ...
            if(count == 60):
                ...
                print()
                thetaL = np.array([[0.0],[random.uniform(-3.14, 3.14)],[random.uniform(-3.14, 3.14)],[random.uniform(-3.14, 3.14)],[random.uniform(-3.14, 3.14)],[random.uniform(-3.14, 3.14)],[random.uniform(-3.14, 3.14)],[random.uniform(-3.14, 3.14)]])
                thetadot = np.array([100])
                TL_1in0 = TtoM(thetaL,SL,ML)
                J = Jmaker(thetaL,SL)
                V0 = dvskew(sl.logm(T_2in0.dot(nl.inv(TL_1in0))))
    ...
    while(True):
        ...
        while nl.norm(V) >=.01 and nl.norm(thetadot) >= .0001:
            ...
            if(count == 60):
                ...
                print("Norm of V could not merge - Using new starting position and trying again")
                print()
                thetaR = np.array([[0.0],[random.uniform(-3.14, 3.14)],[random.uniform(-3.14, 3.14)],[random.uniform(-3.14, 3.14)],[random.uniform(-3.14, 3.14)],[random.uniform(-3.14, 3.14)],[random.uniform(-3.14, 3.14)],[random.uniform(-3.14, 3.14)]])
                thetadot = np.array([100])
                TR_1in0 = TtoM(thetaR,SR,MR)
                J = Jmaker(thetaR,SR)
                V0 = dvskew(sl.logm(T_2in0.dot(nl.inv(TR_1in0))))
...
def graspCube():
    ...
    table = .7
    offset = .013
    top = height*5+table+offset
    first = table+offset+0.06
    third = table+height*2+offset
    ...
    # time.sleep(15)
    # pickup
    close_baxter_gripper("BaxterGripper_closeJoint#0")
    # time.sleep(5)
    getLeftToPoint(False, xstart, ystart, third, slowTravel)
    getLeftToPoint(False, xstart, ystart+.2, third, slowTravel)
    getLeftToPoint(False, xstart, ystart+.2, first, slowTravel)
    # place the cube at new location
    open_baxter_gripper("BaxterGripper_closeJoint#0")
    getLeftToPoint(False, xstart, ystart+.2, third, slowTravel)

def push(start, target):
    '''
    start = [x y], starting position
    target = [x y], ending position
    '''
    xstart = .7
    ystart = 0.0
    height = .1
    ...
    offset = .013
    top = height*5+table+offset
    push_on_table = table+offset
    first = table+offset+0.1
    fastTravel = 2
    midTravel = .1
    slowTravel = .05

    # close_gripper("BaxterLeftGripper")

    # move to preparation point
    getLeftToPoint(False, xstart, ystart, push_on_table, midTravel)
    # move to start point
    x_dist = max(start[0]-target[0], target[0]-start[0])
    y_dist = max(start[1]-target[1], target[1]-start[1])
    if start[0] > target[0]:
        start_x = start[0]+0.05
    else:
        start_x = start[0]-0.05
    if start[1] > target[1]:
        start_y = start[1]+0.05
    else:
        start_y = start[1]-0.05
    getLeftToPoint(False, start_x, start_y, first, slowTravel)
    for i in range(10):
        getLeftToPoint(False, start_x+i*0.1*x_dist, start_y+i*0.1*y_dist, first, slowTravel)
    
    
def close_baxter_gripper(gripper_name):
    # vrep.simxSetIntegerSignal(clientID, gripper_name, 1, vrep.simx_opmode_oneshot)
    # time.sleep(15)
    gripper_motor_velocity = 0.15
    gripper_motor_force = 150
    sim_ret, gripper_handle = vrep.simxGetObjectHandle(clientID, gripper_name, vrep.simx_opmode_blocking)
    sim_ret, gripper_joint_position = vrep.simxGetJointPosition(clientID, gripper_handle, vrep.simx_opmode_blocking)
    vrep.simxSetJointForce(clientID, gripper_handle, gripper_motor_force, vrep.simx_opmode_blocking)
    vrep.simxSetJointTargetVelocity(clientID, gripper_handle, gripper_motor_velocity, vrep.simx_opmode_blocking)
    print("close_baxter_gripper -- gripper_joint_position: ", gripper_joint_position)
    ...
    while gripper_joint_position > 0.005:  # Block until gripper is fully closed
        sim_ret, new_gripper_joint_position = vrep.simxGetJointPosition(clientID, gripper_handle, vrep.simx_opmode_blocking)
...
def open_baxter_gripper(gripper_name):
    # vrep.simxSetIntegerSignal(clientID, gripper_name, 0, vrep.simx_opmode_oneshot)
    # time.sleep(10)

    gripper_motor_velocity = -0.05
    gripper_motor_force = 20
    sim_ret, gripper_handle = vrep.simxGetObjectHandle(clientID, gripper_name, vrep.simx_opmode_blocking)
    sim_ret, gripper_joint_position = vrep.simxGetJointPosition(clientID, gripper_handle, vrep.simx_opmode_blocking)
    vrep.simxSetJointForce(clientID, gripper_handle, gripper_motor_force, vrep.simx_opmode_blocking)
    vrep.simxSetJointTargetVelocity(clientID, gripper_handle, gripper_motor_velocity, vrep.simx_opmode_blocking)
    # print("open_gripper: gripper_joint_position ", gripper_joint_position)
    while gripper_joint_position < 2.0e-05:  # Block until gripper is fully open
        # print("open_gripper: gripper_joint_position ", gripper_joint_position)
        # vrep.simxSetIntegerSignal(clientID, gripper_name, 0, vrep.simx_opmode_blocking)
        sim_ret, gripper_joint_position = vrep.simxGetJointPosition(clientID, gripper_handle, vrep.simx_opmode_blocking)
